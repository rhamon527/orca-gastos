...
def add_gasto(obra_id):
    ...
    num = re.sub(r'[^\d,\.]', '', raw_valor).replace('.', '').replace(',', '.')
    valor = float(num)
    gasto = Gasto(
...
def graficos():
    obras = Obra.query.all()
    return render_template('graficos.html', obras=obras)
...
def api_gastos_tipos():
    ...
    for cat in categorias:
        query = db.session.query(db.func.sum(Gasto.valor)).filter(Gasto.tipo_nota == cat)
        if obra_id:
            query = query.filter(Gasto.obra_id == obra_id)
